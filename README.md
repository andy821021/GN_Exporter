...
The importer is currently split into two practical categories.

### Fully usable now

- general node creation by `bl_idname`
- general links
- group interface reconstruction
- nested group import
- common property restoration
- `GeometryNodeIndexSwitch`
- `GeometryNodeMenuSwitch`

### Detected but not fully reconstructable yet
...
- Bake-related nodes / bake dynamic sockets

These node families may still be created partially, but their zone pairing or dynamic sockets are not yet reliably reconstructed by the importer.
For now, treat them as experimental / partial support rather than production-ready support.
...
The importer currently has experimental / partial handling for:

- `GeometryNodeCaptureAttribute` — `capture_items`
- `GeometryNodeIndexSwitch` — `index_switch_items`
- `GeometryNodeMenuSwitch` — `enum_items`
- `GeometryNodeSimulationInput` / `GeometryNodeSimulationOutput` — `state_items`
- `GeometryNodeRepeatInput` / `GeometryNodeRepeatOutput` — `repeat_items`
- `GeometryNodeBakeNode` — `bake_items`
- `GeometryNodeForeachGeometryElementInput` — `input_items`, `generation_items`
- `GeometryNodeForeachGeometryElementOutput` — `output_items`, `generation_items`

It also retries socket resolution during input assignment and link creation,
including a dynamic-item fallback for `GeometryNodeCaptureAttribute` when the
requested socket does not exist yet.

Important:

- `GeometryNodeIndexSwitch` and `GeometryNodeMenuSwitch` are currently the most reliable dynamic-item node families.
- Simulation / Repeat / For Each / Bake-related handling should still be treated as partial support.
- For zone-based nodes, the importer may create visible nodes without fully reconstructing the true Blender zone pairing state.

Supported JSON fields per node type:

| Node | JSON key | Item fields |
|---|---|---|
| `GeometryNodeCaptureAttribute` | `capture_items` | `name`, `data_type` |
| `GeometryNodeIndexSwitch` | `index_switch_items` | `data_type` |
| `GeometryNodeMenuSwitch` | `enum_items` | `name` |
| `GeometryNodeSimulationInput/Output` | `state_items` | `name`, `socket_type` |
| `GeometryNodeRepeatInput/Output` | `repeat_items` | `name`, `socket_type` |
| `GeometryNodeBakeNode` | `bake_items` | `name`, `socket_type` |
| `GeometryNodeForeachGeometryElementInput` | `input_items`, `generation_items` | `name`, `socket_type` |
| `GeometryNodeForeachGeometryElementOutput` | `output_items`, `generation_items` | `name`, `socket_type` |

All of these also support `dynamic_items.<key>` as an alternative nesting.
...
```json
{
  ...
  "capture_items": [
    { "name": "MyCapturedValue", "data_type": "FLOAT" }
  ]
}
```

`GeometryNodeIndexSwitch` example:
...
`GeometryNodeMenuSwitch` example:
...
```json
{
  ...
  "state_items": [
    { "name": "Position", "socket_type": "NodeSocketVector" },
    { "name": "Velocity", "socket_type": "NodeSocketVector" }
  ]
}
```
...
- for zone-based nodes, JSON structure alone is not yet enough to guarantee exact reconstruction
...
13. For dynamic-item nodes, prefer `GeometryNodeIndexSwitch` and `GeometryNodeMenuSwitch` for reliable tests; zone-based nodes (`Simulation`, `Repeat`, `For Each`) and Bake-related nodes are still partial support
...
- restore dynamic items reliably for `GeometryNodeCaptureAttribute`, `GeometryNodeIndexSwitch`, and `GeometryNodeMenuSwitch`
- attempt partial dynamic-item restoration for Simulation / Repeat / Foreach / Bake-related nodes
...
- reliable reconstruction of zone-based node families:
  - `Simulation`
  - `Repeat`
  - `For Each`
- reliable bake-node reconstruction across Blender versions
